...
| 			  		 |	... instantiating a new TestType? | TestResultStore | **Creator (R1)** and **HC+LC**: By the application of the Creator (R1) it would be the "Company". But, by applying HC + LC to the "Company", this delegates that responsibility to the "TestResultStore"   |
...
| Step 2  		 | 	|	 |	  |
| Step 3 		 |	...saving the inputted data? | TestResult  | **IE:** object created in step 1 has its own data.  |
| 			  		 | ... knowing if the test exists?  | TestStore | **IE:** knows all the tests in the system  |
...
| Step 5  		 |	... saving the selected category? | RecordTestResult  | **IE:** object created in step 1 is classified in one Test.  | 
| Step 6  		 |  |  | | 
| Step 7  		 |	...saving the inputted data?  | RecordTestResult | **IE:** owns its data.|          
| Step 8  		 |	... validating all data (local validation)? | RecordTestResult | **IE:** owns its data.| 
